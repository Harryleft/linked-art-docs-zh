...
```crom
top = model.HumanMadeObject(ident="spring/5", label="马奈的珍妮（春天）")
top.referred_to_by = vocab.Description(content="一位穿着带有花卉点缀日装的时尚年轻女士，在繁茂叶子的背景下撑着阳伞。")
...
```crom
top = model.VisualItem(ident="spring/1", label="春天的视觉内容")
top.represents = model.Person(ident="jeanne", label="珍妮·德马尔西")
```
...
```crom
top = model.VisualItem(ident="spring/5", label="春天的视觉内容")
top.represents_instance_of_type = model.Type(ident="http://vocab.getty.edu/aat/300046218", label="阳伞")
```
...
```crom
top = model.VisualItem(ident="spring/2", label="春天的视觉内容")
top.about = model.Type(ident="http://vocab.getty.edu/aat/300133097", label="春天（季节）")
```
...
```crom
top = model.VisualItem(ident="spring/3", label="春天的视觉内容")
...
```crom
top = model.VisualItem(ident="spring/4", label="春天的视觉内容")
...
top.referred_to_by = vocab.SignatureStatement(content="签名和日期：'Rembrandt f 1642'")
```
...
```crom
top = vocab.ExhibitionCatalog(ident="catalog/1", label="展览目录副本")
top.carries = model.LinguisticObject(ident="catalogtext", label="展览目录文本")
top.shows = model.VisualItem(ident="spring", label="春天的视觉内容")
top.shows = model.VisualItem(ident="houses", label="普罗旺斯房屋的视觉内容")
```
...
```crom
top = vocab.Print(ident="harpers/1", label="海报项目")
vi = model.VisualItem(ident="harpers", label="Harpers的视觉内容")
top.shows = vi
```

```crom
top = model.VisualItem(ident="harpers/1", label="Harpers的视觉内容")
top.represents = model.Type(ident="http://vocab.getty.edu/aat/300025943", label="女性")
top.referred_to_by = vocab.Description(content="文本和图像主要是红色和黑色")
```

```crom
top = model.LinguisticObject(ident="harpers/1", label="Harpers的文本组件")
top.content = "Harper's. January contains Roden's corner. A Novel by Henry Seton Merriman [...]"
top.part_of = model.VisualItem(ident="harpers", label="Harpers的视觉内容")
```
